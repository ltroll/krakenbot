...
- `range_grid_strategy_price_first_source_policy_candidate.json`
  Paper-only comparison profile. Low entries are price-first and weather-sized, median entries prefer stabilization but can probe at reduced size, and high entries require confirmed range chop. It also assigns wider low spacing, tighter high spacing, source-specific profit targets, and separate inventory caps. It is included in `range_grid_strategy_test_set.txt` but is not selected by `env.range`.
