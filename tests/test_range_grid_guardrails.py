...
class RangeGridGuardrailsTests(unittest.TestCase):
    ...
    def test_validate_strategy_config_rejects_out_of_bounds_risk_context_threshold(self):
        errors = guardrails.validate_strategy_config({
            ...
            "stale_level_reanchor_profit_guard_low_dip_min_resistance_room_pct": 1.5,
        ...
        self.assertTrue(
            any(
                "stale_level_reanchor_min_entry_opportunity_score" in error
                for error in errors
            )
        )
        self.assertTrue(
            any(
                "stale_level_reanchor_max_exit_pressure_score" in error
                for error in errors
            )
        )
        self.assertTrue(
            any(
                "stale_level_reanchor_profit_lookback_hours" in error
                for error in errors
            )
        )
        self.assertTrue(
