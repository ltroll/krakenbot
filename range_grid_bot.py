...
LOG_ROTATE_RETENTION = int(
    os.getenv("RANGE_GRID_TRADE_LOG_RETENTION", "8")
)
ACTIVITY_LOG_FILE = (
...
def source_weather_entry_guard(config, fallback_config, buy_source, weather_report):
    ...
    blocked_postures = config_token_set_with_fallback(
        config,
        fallback_config,
    ...
    hold_through_score = optional_float(opportunity.get("hold_through_score"))
    stabilization_score = optional_float(stability.get("stabilization_score"))
    min_stabilization = strategy_float_with_fallback(
        config,
        fallback_config,
    ...
    min_hold_through = strategy_float_with_fallback(
        config,
        fallback_config,
    ...
    max_median_reclaim_distance_pct = strategy_float_with_fallback(
        config,
        fallback_config,
        "weather_entry_guard_max_median_reclaim_distance_pct",
        0.0,
    )
    median_reclaim_distance_pct = optional_float(
        weather_resistance_room_by_type(weather_report, "median_reclaim")
    )
    if (
...
def weather_actionable_resistance(weather_report):
    location = weather_market_location(weather_report)
    current_price = optional_float(location.get("current_price"))
...
def allow_above_last_sell_for_candidate(config, buy_source, weather_report):
    ...
    breakout_score = optional_float(risk_context.get("breakout_score")) or 0.0
    rebound_score = optional_float(risk_context.get("rebound_score")) or 0.0
...
def stale_reanchor_profit_guard_log_fields(stale_reanchor):
    ...
    return {
        ...
        "stale_level_reanchor_profit_guard_bypass_resistance_type": (
            (bypass.get("actionable_resistance") or {}).get("type")
        ),
        "stale_level_reanchor_profit_guard_bypass_resistance_distance_pct": (
            (bypass.get("actionable_resistance") or {}).get("distance_pct")
        ),
        "stale_level_reanchor_profit_guard_bypass_min_resistance_room_pct": (
            bypass.get("min_resistance_room_pct")
        ),
    }
