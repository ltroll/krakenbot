...
def source_weather_entry_guard(config, buy_source, weather_report):
    if not strategy_bool(config, "weather_entry_guard_enabled", False):
    ...
    trend_pressure = weather_trend_pressure(weather_report)
    if strategy_bool(config, "weather_entry_guard_block_falling_tape", True):
        if bool(trend_pressure.get("falling_tape")):
    ...
    max_exit_pressure = strategy_float(
        config,
    ...
    nearest_support = weather_nearest_support(weather_report)
    support_distance_pct = safe_float(nearest_support.get("distance_pct"))
    max_support_distance_pct = strategy_float(
        config,
    ...
    resistance_room = weather_actionable_resistance(weather_report)
    resistance_room_pct = safe_float(resistance_room.get("distance_pct"))
    min_resistance_room_pct = strategy_float(
        config,
